...
# run and detach
("$QEMU_BIN" -nographic -machine "$ENV_CHIP" "${PSRAM_ARGS[@]}" -drive "file=$QEMU_FLASH_IMAGE,if=mtd,format=raw" -global "driver=timer.$ENV_CHIP.timg,property=wdt_disable,value=true" | tee -a "$log_file") &
